...
SELECT a.Location,a.`Total Doses Administered Daily`, a.`Date` FROM vaccination_data AS a
	INNER JOIN(SELECT Location, MAX(`Total Doses Administered Daily`) AS vaccinations FROM vaccination_data GROUP BY Location) AS b
    ON a.Location = b.Location AND a.`Total Doses Administered Daily` = b.vaccinations